---
name: ayjnt-rpc
description: Add typed agent-to-agent or browser-to-agent calls in Ayjnt. Use for inter-agent RPC, `this.agent(TargetClass, name)`, callable methods, `agent.stub.method()`, or catalog exposure. Prefer class-valued peer lookup inside Ayjnt Agent classes, use `getAgent(namespace, name)` only outside that base class or with a custom namespace, and use `@callable()` for browser RPC and catalog metadata.
---
...
|---|---|---|---|
| **`this.agent(TargetClass, name)`** | Another Ayjnt agent | Generated class-to-binding registry + native DO RPC | None — TypeScript `public` is enough |
...
## Pattern 1 — agent-to-agent: class-safe lookup

When one Ayjnt agent calls another, import the target class as a value
and pass it to `this.agent`. The class supplies both TypeScript
autocomplete and the runtime-safe key for the generated binding map.
No decorator or binding string is required.
...
export default class InventoryAgent extends Agent<State> {
  ...
  // Plain public method — callable by other agents.
...
export default class OrdersAgent extends Agent {
  override async onRequest(request: Request): Promise<Response> {
    ...
    try {
      const inv = await this.agent(InventoryAgent, "main");
...
- Import the target class as a **value**, not `import type`. The
  constructor is intentionally present at runtime.
- Default-export every agent from `agent.ts` and run `ayjnt build`
  after adding or moving one so the constructor registry regenerates.
- If a class is missing from the registry, do not fall back to a
  guessed string; fix discovery or the default export.
- Awaiting every call. Each one is a DO round-trip, possibly on
  another machine.
- Args must be structured-cloneable. No functions; serialise `Date`,
  `Map`, `Set` yourself.

### Outside an Ayjnt Agent

Use `getAgent(namespace, name)` when code does not extend Ayjnt's
`Agent`, or when deliberately using a custom Durable Object namespace:

```ts
import { getAgent } from "ayjnt/rpc";

const inventory = await getAgent(env.INVENTORY_AGENT, "main");
await inventory.decrement("widget", 2);
```

The namespace carries the stub type; no explicit generic is normally
needed.
...
`agent.stub.<method>` autocompletes with every callable method.
Prefer the stub over a string-based `agent.call(...)`.
...
- **Import `callable` from `"ayjnt"`** for the canonical Ayjnt surface.
  It is the upstream Cloudflare decorator, re-exported unchanged.
...
browser exposure — typically a method other agents call through
`this.agent(TargetClass, name)` that you still want advertised — use the legacy JSDoc
...
|---|---|---|---|---|
| ✗ | ✗ | ✗ | ✗ | Internal helpers. Other agents can still call via a class-safe peer stub — visibility is just TypeScript's `public`. |
...
Agent-to-agent RPC works against any public method regardless of
decoration — it does not go through the browser WebSocket or catalog.
...
- [`examples/orchestration`](../../../examples/orchestration) — typed
  class-valued calls across lead, researcher, and reviewer agents.
