...
| `--no-embed-workerd` | Leave workerd out. The binary then needs `AYJNT_WORKERD_PATH`. |
| `--bytecode` | Precompile to bytecode for faster startup. |
| `--minify` | Minify the embedded JavaScript. |

The default output is large because it contains Bun, a platform workerd
binary, and the application. Exact size varies by platform and release.
What it needs to run: no Bun, `node_modules`, or wrangler.
